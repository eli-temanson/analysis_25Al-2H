...
void Analyzer::ProcessEvent(){
    ...
    if(true){
        ...
        ic.ReconstructHitPos(ic.xcluster.ChRaw(),ic.ycluster.ChRaw());
        if(!InsideGate(ic.ycluster.E()-ic.xcluster.E(),591,1450)) return; 
        if(!InsideGate(ic.xcluster.E(),886,1482)) return; 

        //MyFill(table0,"DS_ic_xy_cluster",32,0,32,ic.xcluster.Ch(), 32,0,32,ic.ycluster.Ch());
        //MyFill(table0,"DS_ic_xy",1000,-50,50,ic.HitPos.X(), 1000,-50,50,ic.HitPos.Y());
        ...
        if(InsideGate(ic_p1,ic.ESeg.ERaw(),ic.DESeg.ERaw())) // selection the 25Al beam
        {
          ...
          //MyFill(table0,"DS_IonC_xy_cluster_p1",64,0,32,ic.xcluster.Ch(), 64,0,32,ic.ycluster.Ch());
          MyFill(table0,"DS_IonC_xy_cluster_p1",64,0,32,ic.xcluster.ChRaw(), 64,0,32,ic.ycluster.ChRaw());
          MyFill(table0,"DS_IonC_xy_p1",1000,-50,50,ic.HitPos.X(), 1000,-50,50,ic.HitPos.Y());
